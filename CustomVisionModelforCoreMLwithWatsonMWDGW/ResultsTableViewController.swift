...
class ResultsTableViewController: UITableViewController {
    ...
    var classifiers: [VisualRecognitionV3.ClassResult] = [] {
        didSet {
            isInitialized = true
        }
    }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        guard classifiers.count > 0, let score = classifiers[0].score, score > 0.5 else {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.classificationLabel.text = String((classifier.score?.truncate(places: 2) ?? 0.0))
        ...
        let width = Double(cell.frame.width) * (classifier.score ?? 0.0)
        cell.scoreScale.frame = CGRect(x: 0, y: 0, width: width, height: Double(cell.frame.height))
        cell.scoreScale.updateConstraints()

        let color = (classifier.score ?? 0.0) >= 0.6 ? UIColor(rgb: 0xAEEA00).withAlphaComponent(0.5) :
